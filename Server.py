    ...
    if request.method == 'POST':
        ...
        while i < (dic_length-2):
            ...
            if type == "Multiple_Choice":
                ...
                b = {"choice_B": dict.get(key_list[i + 5])}
                quiz = {}
                for d in (question, question_type, answer, point, a, b):
                    quiz.update(d)
                full_quiz.append(quiz)
                i += 5
        ...
        if dict.get('build quiz') is None:
            ...
            for q in full_quiz:
                ...
                js_template += '"<br>"+\n'
                js_template += '"<label>ChoiceA <input type = \\"text\\" name = \\"Choice_A_'+str(i)+'\\" size=\\"120\\"/></label >" +\n'
                js_template += '"<label>ChoiceB <input type = \\"text\\" name = \\"Choice_B_'+str(i)+'\\" size=\\"120\\"/></label >" +\n'
                js_template += '"<label>ChoiceC <input type = \\"text\\" name = \\"Choice_C_'+str(i)+'\\" size=\\"120\\"/></label >" +\n'
                js_template += '"<label>ChoiceD <input type = \\"text\\" name = \\"Choice_D_'+str(i)+'\\" size=\\"120\\"/></label >" ;\n'
                js_template += '}'+'\n'
                ...
                js_template += '"<textarea class=\\"form-control\\" name=\\"Answer_'+str(i)+'\\" rows=\\"5\\" id=\\"comment\\"></textarea></div>";\n'
                js_template += '}'+'\n'
                js_template += 'else{'+'\n'
                js_template += 'document.getElementById("question_content'+str(i)+'").innerHTML = \n'
                js_template += '"<label>Answer : <input type = \\"text\\" name = \\"Answer_'+str(i)+'\\" size=\\"12\\"/></label >" +\n'
                ...
                js_template += '"<br>" +\n'
                js_template += '"<input type=\\"radio\\" id=\\"T\\" name=\\"T\\" value=\\"True\\">" +\n'
                js_template += '"<label for=\\"T\\">True</label>" +\n'
                js_template += '"<br>" +\n'
                js_template += '"<input type=\\"radio\\" id=\\"F\\" name=\\"F\\" value=\\"False\\">" +\n'
                ...
                if q.get("type") == "Multiple_Choice":
                    quiz_template += '<label> Question' +str(i) + ' <input type = "text" name = "Question_' + str(i)+ '" size="120" value="' + q.get("question")[0] + '"/></label >' + "<br>" +"\n"
                    ...
                    quiz_template += '<label>Answer : <input type = "text" name = "Answer_' + str(i) + '" size="12"/></label ><br>' + '\n'
                    ...
                    quiz_template += '</p>' + '\n'
                    quiz_template += '<label>ChoiceA <input type = "text" name = "Choice_A_' + str(i) + '" size="120" /></label >' + '\n'
                    quiz_template += '<label>ChoiceB <input type = "text" name = "Choice_B_' + str(i) + '" size="120" /></label >' + '\n'
                    quiz_template += '<label>ChoiceC <input type = "text" name = "Choice_C_' + str(i) + '" size="120" /></label >' + '\n'
                    quiz_template += '<label>ChoiceD <input type = "text" name = "Choice_D  _' + str(i) + '" size="120" /></label >' + '\n'
                    quiz_template += '</p>'  + '\n'
                    ...
                    quiz_template += '<label for="option'+str(i)+'">Question Type :</label>' + '\n'
                    quiz_template += '<select name="question_type" id="option'+str(i)+'" value="True_or_False" >' +'\n'
                    ...
                    quiz_template += '<p id="question_content'+str(i)+'">' + '\n'
                    quiz_template += '</p>' + '\n'
                    quiz_template += '<label>True <input type = "text" name = "Choice_A_' + str(i) + '" size="120" /></label >' + '\n'
                    quiz_template += '<label>False <input type = "text" name = "Choice_B_' + str(i) + '" size="120" /></label >' + '\n'
                    quiz_template += '</p>' + '\n'
...
@app.route('/rubric', methods=['POST', 'GET'])
def addRubric():
    if request.method == 'GET':
        return render_template('rubic.html')
